...
const GenerateQR = async (req: any, res: any) => {
  ...
  try {
    ...
    const qrData: string = `https://l59b0rw5-4200.inc1.devtunnels.ms/`;
    ...
    res.json({
      qrCodeUrl: `https://qrbackend-5.onrender.com/${email}.png`,
      email: email,
      success: true,
    });
  } catch (error: any) {
    ...
    res
      .status(500)
      .json({ error: "Failed to generate QR code", success: false });
  }
};
...
const GetMenuByEmail = async (req: any, res: any) => {
  try {
    ...
    if (!nonVegMenu.length && !vegMenu.length) {
      return res
        .status(404)
        .json({ message: "No menu found for this email", success: false });
    }
